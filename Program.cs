/*
 * ─────────────────────────────────────────────────────────────────
 *  OBJ Viewer — Silk.NET  |  with built-in file picker
 *
 *  Made by BenyVK
 *  Source: https://github.com/BenyVK/cs-object
 *
 *  Controls:
 *    Left click + drag   → Rotate
 *    Right click + drag  → Pan
 *    Scroll              → Zoom
 *    O                   → Open OBJ file (native OS dialog)
 *    R                   → Reset camera
 *    Escape              → Quit
 *
 *  Dependencies (csproj):
 ...
 *      or, if Windows-only:
 *    <PackageReference Include="System.Windows.Forms"   Version="*" />  (net-windows TFM only)
 *
 *  If neither package is installed, falls back to Console.ReadLine().
 * ─────────────────────────────────────────────────────────────────
 */
...
namespace ObjViewer
{
    ...
    internal static class FileDialog
    {
        ...
        public static string? OpenObjFile(string? startDir = null)
        {
            ...
            // ── 3. macOS: open via shell (osascript) ───────────────────────────
            ...
            // ── 5. Console fallback ─────────────────────────────────────────────
            Console.Write("[FileDialog] Enter path to OBJ file: ");
    ...
    public class ObjViewer
    {
        ...
        public void Run()
        {
            ...
            opts.Title = "OBJ Viewer  |  O = Open file   R = Reset   Esc = Quit  |  by BenyVK";
        ...
        private void OnKeyDown(IKeyboard kb, Key key, int _)
        {
            ...
            {
                case Key.O:
                    // Open dialog on a background thread.
                    // STA apartment state is only meaningful/supported on Windows.
                    var t = new Thread(() =>
                    {
        ...
        private unsafe void OnRender(double _)
        {
            ...
            if (toLoad != null)
            {
                ...
                _window.Title = $"OBJ Viewer  |  {Path.GetFileName(toLoad)}  |  O=Open  R=Reset  Esc=Quit  |  by BenyVK";
            }
        ...
        private unsafe void SetupShader()
        {
            ...
            uint Compile(ShaderType type, string src)
            {
                ...
                if (ok == 0)
                {
                    string log = _gl.GetShaderInfoLog(sh) ?? "(no shader log available)";
    ...
    class Program
    {
        // Made by BenyVK — https://github.com/BenyVK/cs-object
        [STAThread]
